  ...
  >
    <template #default="{ hide }">
      ...
      <div class="d-flex justify-content-between align-items-center content-sidebar-header px-2 py-1">
        <h5 class="mb-0">
              <span v-if="isEdit">Update Customer</span>
...
<b-col
        ...
        >
          ...
          >
            ...
            >
            <v-select
                  v-model="userData.customer_type"
                  :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                  label="title"
...
export default {
  ...
  methods: {
    ...
    getAddressData(addressData, placeResultData, id) { 
      ...
      this.userData.address = addressData.route;
      },
    handleError(error) {
        alert(error)
      },
    ...
    onSelect({name, iso2, dialCode}) {
       ...
       console.log(localStorage["dialCode"]);
        ...
        formSubmit(e) {              
                ...
                var phonumber = localStorage["dialCode"]+this.userData.phone;                
                let url=''
                console.log('submit is edit',this.isEdit)
                if(this.isEdit){     
                            
                  url = 'http://koboautos.com/twic-terminal/connect/api/update_customer?key='+this.userData.key+"&";
                }
                else{
                  ...
                  axios({
                    method: 'post',
                    url: url+'first_name=' + this.userData.first_name+'&last_name=' + this.userData.last_name+'&email=' + this.userData.email+'&phone='+ this.userData.phone+'&gender='+this.userData.gender+'&customer_type='+this.userData.customer_type+'&address='+this.userData.address+'&account_type='+this.userData.account_type+'&company_name='+this.userData.company_name,
                })
    ...
    allcustomersgroups(e) { 
      axios.post('http://koboautos.com/twic-terminal/connect/api/create_group')
        .then(response => {
         console.log("customer types",response);
    ...
    watch:{
      'userData.account_type'  : function (val, oldval) {
